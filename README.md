...
- 实时展示币种最新价格
- 首页改为桌面客户端式行情工作台，默认使用左侧关注列表 + 右侧市场状态面板，减少主窗口长滚动
- 主窗口默认使用更适合桌面盯盘的宽高，并设置最小尺寸，避免核心文字和按钮被挤压截断
...
- 首页增加独立“市场情绪”视图，支持查看不同区间下的情绪曲线变化
...
- 关于页支持展示工具简介、当前版本、GitHub 项目主页入口，以及基于 GitHub Release 的应用内更新包下载按钮
- WebSocket 断开后自动重连
...
关于页的更新按钮会读取 GitHub Release 最新版本，自动匹配当前系统安装包，并下载到系统“下载”目录；下载完成后会尝试直接打开安装包，方便继续安装升级。
