...
- 首页采用桌面客户端式工作台布局，默认将关注币种放在左侧列表、市场状态放在右侧面板，降低主窗口整体滚动压力。
- 首页额外提供“市场情绪”视图，用于展示 Alternative.me Fear & Greed Index 的当前分数和历史曲线。
...
- 首页默认不再把市场总览、情绪曲线和所有币种大卡片纵向堆叠；关注列表在桌面工作台左侧独立滚动，情绪曲线通过视图切换进入。
- 首页右侧只保留市场状态相关内容，不再放置纯说明型大面板，减少无效占位。
...
- 关于页更新按钮会先检查 GitHub Release 最新版本；如果有新版本，会在应用内下载对应系统安装包，下载完成后尝试打开安装包。
